...
public class FourPointSurrogateD<T extends CartesianPoint> implements
		Metric<double[]> {
	 ...
	 * @param refPoints
	 */
	public FourPointSurrogateD(Metric<T> m, List<T> refPoints) {
		this.refPoints = refPoints;
		this.metric = m;
		int nChoose2 = refPoints.size() * (refPoints.size() - 1) / 2;
		this.distanceTable = new double[nChoose2];
		int ptr = 0;
		for (int i = 0; i < refPoints.size() - 1; i++) {
			for (int j = i + 1; j < refPoints.size(); j++) {
				distanceTable[ptr++] = m.distance(refPoints.get(i),
						refPoints.get(j));
			}
		}
