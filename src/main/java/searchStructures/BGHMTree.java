...
import searchStructures.experimental.PermutationTree.PNode;
 ...
 */
public class BGHMTree<T> extends SearchIndex<T> {
	...
	private class HeadNode {
		...
		HeadNode(List<T> data) {
			...
			this.l_r_dist = BGHMTree.this.metric.distance(this.lHead,
					this.rHead);
		...
		public void thresholdSearch(T query, double t, List<T> res) {
			...
			if (this.lHead == null) {
				...
				if (BGHMTree.this.metric.distance(query, this.lHead) <= t) {
					res.add(this.lHead);
				}
			} else {
				dLeft = BGHMTree.this.metric.distance(query, this.lHead);
				dRight = BGHMTree.this.metric.distance(query, this.rHead);
	...
	private class TreeNode {
		...
		TreeNode(T upperNode, List<T> data, int depth) {
			BGHMTree.this.maxDepth = Math.max(BGHMTree.this.maxDepth, depth);
			this.lCR = 0;
			this.rCR = 0;
			if (data.size() == 0) {
				//
		...
		public void thresholdSearch(T query, double t, double upperDist,
			...
			if (this.lHead == null) {
				...
				if (BGHMTree.this.metric.distance(query, this.rHead) <= t) {
					res.add(this.rHead);
				}
			} else {
				dLeft = upperDist;
	...
	private BGHMTree<T>.HeadNode head;
	...
	public BGHMTree(List<T> data, Metric<T> metric, boolean hilbertMetric) {
		super(data, metric);
		...
		if (this.hilbertMetric) {
			ss = new SemiSorter<T>(data) {
				...
				public double measure(T d) {
					double d1 = BGHMTree.this.metric.distance(d, left);
					double d2 = BGHMTree.this.metric.distance(d, right);
